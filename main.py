...
async def jiosaavn(_, message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"https://jiosaavnapi.bhadoo.uk/result/?query={query}") as resp:
    ...
    draw.text((190, 560),f"Title - {sname}-{ssingers}",(255,255,255),font=font)
    draw.text((190, 620),f"Duration - {sduration} Seconds",(255,255,255),font=font)
    draw.text((190, 650),f"Group - {message.chat.username}-{message.chat.id}",(255,255,255),font=font)
    draw.text((190, 680),f"Played By - {message.from_user.username}-{message.from_user.id}",(255,255,255),font=font)
    img.save('final.png')
    os.system("rm temp.png")
    os.system("rm background.png")
    await m.delete()
    m = await message.reply_photo(caption="Playing", photo="final.png",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(f"STOP {emoji.CROSS_MARK}",
                                         callback_data="end")
                ]
            ]
        ),
...
@app.on_message(filters.command(["ytplay"]) & filters.chat(sudo_chat_id) & ~filters.edited)
async def ytplay(_, message: Message):
    ...
    draw.text((190, 620),f"Duration - {duration}",(255,255,255),font=font)
...
@app.on_message(filters.command(["tgplay"]) & filters.chat(sudo_chat_id) & ~filters.edited)
async def playlist(_, message: Message):
    global blacks
    if message.from_user.id in blacks:
        await message.reply_text("You're Blacklisted, So Stop Spamming.")
    ...
    f = await app.download_media(message.reply_to_message, file_name="audio.mp3")
    await m.edit("Playing")
    s = await asyncio.create_subprocess_shell("mpv downloads/audio.mp3 --no-video", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    await s.wait()
    await m.delete()
...
async def end_callback(_, CallbackQuery):
    ...
    try:
        ...
        pass
    await app.send_message(CallbackQuery.message.chat.id, f"{CallbackQuery.from_user.mention}{CallbackQuery.from_user.id} Stopped The Music.")
